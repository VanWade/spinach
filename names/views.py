...
def index(request):
    ...
    context = dict(
        ...
        data=data,
        datas=datas,
        onresize=" <script>  window.onresize = function () {  %s %s   };  </script>" % (
            pyecharts_add(pro.render_embed())[1], pyecharts_add(pro2.render_embed())[1])
    )
...
def cmd_historys(request):
    ...
    return render(request, "names/cmd-history.html",
                  {"historys": obj, "autoops_active": "active", "login_log_active": "active", })
