...
def sql_rb(user, password, host, port, sequence, backup_dbname):  ##   查询回滚语句的表格
    ...
    try:
        with connection.cursor() as cursor:
            ...
            sql = " select  tablename   from  {0}.{1}  where opid_time ='{2}' ;".format(backup_dbname,
        ...
        try:
            with connection1.cursor() as cursor1:
                sql = " select rollback_statement  from  {0}.{1}  where opid_time = '{2}' ;".format(backup_dbname,
                                                                                                    table, sequence)
                cursor1.execute(sql)
...
def Inception_query(request):  ##查询数据库
    ...
    if request.method == 'POST':
        ...
        for i in obj:
            try:
                history.objects.create(ip=i.ip, root=i.db_user.username, port=i.port, cmd="查询:{}".format(sqls),
                                       user=user)
    ...
    if request.method == 'POST':
        ...
        for i in obj:
            try:
                ...
                s = sql(user=i.db_user.username, password=password, host=i.ip, port=i.port, databases=databases,
                        sqls=sql_db)
    ...
    if request.method == 'POST':
        ...
        for i in obj:
            try:
                ...
                password = decrypt_p(i.db_user.password)
                s = sql_exe(user=i.db_user.username, password=password, host=i.ip, port=i.port, databases=databases,
                            sqls=sql_db)
